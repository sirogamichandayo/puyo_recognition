...
void State::paddingImg(const cv::Mat &img_, cv::Mat &img_pad, const float &size)
{
	if (size <= 0|| 0.5 <= size)
		LOG("Out of range");
		std::exit(0);
	...
	std::cout << "(cols, rows) : " << cols << ", " << rows;
	std::cout << "(x, y, width, height) : " << x << ", " << y << ", " << width << ", " << height << std::cout;
