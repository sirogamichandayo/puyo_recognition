...
void State::toHDImg(cv::Mat *const img_)
{
	cv::resize(img_, img_, cv::Size(), 
						static_cast<double>(pic::HD_WIDTH / img_.cols), static_cast<double>(pic::HD_HEIGHT / img_.rows));
...
void State::paddingImg(cv::Mat *const img_, const float &size)
{
	if (size <= 0|| 0.5 <= size)
		LOG("Out of range");
		std::exit(0);
	...
	int x = cols * size;
	int y = rows * size;
	int width = cols * (1 - size*2);
	int height = rows * (1 - size*2);
...
int State::toGetPuyoColorPerPiece(const cv::Mat &image)
{
	...
	std::map<int, int>
			color_pixel_dict = {
					...
					{color::PURPLE, 0}};
	cv::Mat resize_image;
	cv::resize(image, resize_image, cv::Size(), size, size);
