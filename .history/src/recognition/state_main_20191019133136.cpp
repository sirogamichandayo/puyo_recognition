...
#if true
	std::map<std::string, int> color_count_debug;
	for (const auto &[color, count] : color_pixel_dict)
	{
		std::string title;
		colorNum2ColorString(color, &title);
		color_count_debug[title] = count;
	}
	debug::saveElem(color_count_debug.begin(), color_count_debug.end(), "elem");
#endif	
