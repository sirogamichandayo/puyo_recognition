				...
				std::array<int>::iterator first, std::array<int>::iterator end)
{
	...
	if (mode == get_mode::allPuyo_1p)
	{
		...
		getBoard_1p(first, std::step(first, BOARD_ROWS_NO_IN_1314*BOARD_COLS-1));
		std::advance(first, BOARD_ROWS_NO_IN_1314*BOARD_COLS);
		getNext_1p (first, std::step(first, NEXT1_ROWS*NEXT1_COLS-1));
		std::advance(first, NEXT1_ROWS*NEXT1_COLS);
		getNext2_1p(first, std::step(first, NEXT2_ROWS*NEXT2_COLS-1));
