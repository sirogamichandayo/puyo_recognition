...
void State::cut_img(const cv::Mat &img_)
{
...
void State::getNext2_2p(std::vector<int> *field)
{
	...
	for (int i = 0; i < size; ++i)
		(*field)[i] = colorNum2ForBitNum(toGetPuyoColorPerPiece(&img_next2_2p_vec[i]));
}
...
void State::getBoard_2p(std::vector<int> *field)
{
	...
	for (int i = 0; i < size; ++i)
		(*field)[i] = colorNum2ForBitNum(toGetPuyoColorPerPiece(&img_board_2p_vec[i]));
}

int State::GetColor(const cv::Mat *const img)
{
