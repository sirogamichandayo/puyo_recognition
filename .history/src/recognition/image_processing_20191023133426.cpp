...
// if you want get hsv element from color image. elem_size = 3.
//                 gray element from gray image. elem_size = 1.
void img_p::img2ElemPerPixelForVec(const cv::Mat &img_,
																...
																int elem_size)
{
	...
	{
		int index = y;
		const cv::Vec3b *p = &img_.at<cv::Vec3b>(y, 0);
		for (int x = 0; x < cols; ++x, ++p, ++index)
		{
			elem_vec[index].resize(elem_size);
			for (int i = 0; i < elem_size; ++i)
				elem_vec[index][i] = (int)(*p)[i];
		}
	}
}																