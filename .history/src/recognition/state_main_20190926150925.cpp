		...
		std::vector<int>::iterator first, std::vector<int>::iterator end)
{
	...
	if (mode == get_mode::allPuyo_1p)
	{
		...
		auto it = first;
		getBoard_1p(it, std::next(it, BOARD_ROWS_NO_IN_1314*BOARD_COLS-1));
		std::advance(it, BOARD_ROWS_NO_IN_1314*BOARD_COLS);
		getNext_1p (it, std::next(it, NEXT1_ROWS*NEXT1_COLS-1));
		std::advance(it, NEXT1_ROWS*NEXT1_COLS);
		getNext2_1p(it, std::next(it, NEXT2_ROWS*NEXT2_COLS-1));
	...
	if (mode == get_mode::allPuyo_2p)
	{
		...
		auto it = first;
		getBoard_2p(it, std::next(it, BOARD_ROWS_NO_IN_1314*BOARD_COLS-1));
		std::advance(it, BOARD_ROWS_NO_IN_1314*BOARD_COLS);
		getNext_2p (it, std::next(it, NEXT1_ROWS*NEXT1_COLS-1));
		std::advance(it, NEXT1_ROWS*NEXT1_COLS);
		getNext2_2p(it, std::next(it, NEXT2_ROWS*NEXT2_COLS-1));
