...
void State::paddingImg(const cv::Mat &img_, cv::Mat &img_pad, const float &x_size, const float &y_size, const float &width_size, const float &height_size)
{
	if (!(0.0 < size && size < 0.5))
	{
		LOG("Out of range!");
		std::exit(0);
	}
...
int State::toGetPuyoColorPerPiece(const cv::Mat &image)
{
	...
	paddingImg(image, image_padding, 0.2);
