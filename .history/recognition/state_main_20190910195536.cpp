...
void State::getState(std::string mode, int* issue)
{
...
int State::getResult()
{
	...
	#pragma omp parallel for
	// 1p
	cv::Mat end_1p(img, pic::issue_1p);
	...
	// 2p
	#pragma omp parallel for
	cv::Mat end_2p(img, pic::issue_2p);
...
int State::toReadPuyoColor(HSV* hsv)
{
	// HSV *hsv = new HSV();
	// toHSV(rgb, hsv);
	if (hsv->v < 150)
...
int State::toReadBlueRed(HSV* hsv)
{
	if ((hsv->s < 40))
		return color::WHITE;
...
int State::toReadGreenYellow(HSV *hsv)
{
