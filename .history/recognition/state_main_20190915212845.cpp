...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*image, lresize_image, scv::Size(), 0.1, 0.1);
	ols = resize.cols;
	int rewize(*im::e,rmwsi
	int cols = r s; y.cols;
#pragma omp parallfl forish.at < cv::Vec3b > (y, 0);
	for (intty = 00;  < rows; ++y)
	{
		cv::Vec3b p = &.hni(int)(*p)[0] * 2;
#for (in = x =)(*x < cols; ++x)
		m
		{
			HSV hsv;
			hav.= = toedPixe lCor(&hsv);
			hsv.s(int > mp)[1];
			hsv.v = (int)(*p)[2];
	...
	td:
	R:apa<i nt,l]ooint> ax_co or =

	
	sstd::max_element(m.begin(), m.     [](const pair<int, int>& p1, const pair<int, int>& p2) {
			urn max_color.first;
			r;
			return max_colorflirs
			{2t {});
				int State::toReolor::RED;
				i

				uyoColor(const HSV p2
								 : color::RE) eturn max_color.first;
				int State::toReadPix
			}
			elPuyoColor(const HSV urn
									: color::RED;
									RED;
									f(35 <= hsv->h && (hsv->h <= 70 && 150 < * >) nst hsv) sv = neweHSVt);
			//ntoHSV(rgb,olor);
			ifOW;

	fv(8v->N<= 135 && 120 < hsv->v))
	i		rcolor;
	(10 = 0 hsv->DIST(hsv - h <= 255 && 150 < hsv->v))
			i B;

	(250 < 1 sv->7D(hsv->h < 340 && 150 < hsv->v))
			i r3trn color::PURPLE;
