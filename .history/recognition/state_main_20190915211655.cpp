...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*irage, lllllllresize_it go, ssssssscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
...
return	o lo370::Por::U
	...
	:Pr	ol<urn hR60 <		r<urn:hR60 r S V *cop2t8
		{
		rn
				: olir::RED hsv)
											 RED;
...
((1628RPIE
			rn
					:holir::RED && hsv->h <= 30)) && hsv->v > 60)
											 RED;
							return o34orYELL:WUPD
r		eur 85 <= n colh && (hov->hr:=:13OTER;
		
ii5	335(n -Stt::toR&P UL(Pi((3n Stte::toR&P UL(PGe;
v)(160		<lo07=::PhsGRU

	f ((16EENa&& 4te::to5ead2l5ueRed(const HSV *const hsv)
15{
		if85 <= )YELL(Wh && (hsv->hv-=>13 BU<40))
				reu2r0n color::WHITDnt Stat=e:60) || (t <= hsv->hoRe hsv->h <=a3r))e&&nYellow > 60const HSV *const hsv)
		{
f5v)(160 <= hsGREEN5&& 0<h34-THhPU2P5  (	return color::YELLOW;
15			if (tte::toR&P UL(PGe;
n(Sa490 )< h5eadBuBLUevd(const HSV *const hsv)
				2	0					if ((hsv->s < 40))
r>turn o34or::PUWPIGei((3n Stte::toReGeD				
		(		 (tte::toR&PeUL5PGe;
intSa4un c h5eadBluesvd(const HSV *const hsv)
{
									ifo((o Y >s < 40))
			rTturn color::WHITHL:
...
( 6(5(3 Stt:toReGe
		...
		reu75)0 <r>vnHR) &;
 6i((3n Stte::toReGeD
			if (=0<6() || ( 90= hsv->h<   hsv->hh<=s3))>&&) &&& ( hl 60v->hor:7GR && hsv->v > 200EEN;
	
														 THhR) &;

									if ((90 < hsv->					 &	retu5 <0 h sv->Gne & nc&llow(hlv->
cor:7GRO: 
													lor
											: Y > v E 200EEN;
								
		ret50 < urn cLL && (hov->hlo : 7Y:O	return color:YELLO:(hsv->h < 120) && hsv->v > 100)
												;
			}
		}		:	R
	...
	R(320 < hsv->h && hsv->h <= 360) || (0 <= hsv->h && hsv->h <= 30)) && hsv->v > 60)
		return color::RED;
	return color::OTHER;
}
