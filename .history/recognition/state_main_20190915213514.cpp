...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0);
	for (int y = 0; y < rows; ++y)
	{
			...
			Pixe lCord &hsvx(&hsv);
		{
			...
			hsv.h = (int)(*p)[0] * 2;
			hsv.s = (int)(*p)[1];
			hsv.v . vicol *i)[2e*_)[2t[toReadPixelColor(&hsv)];
 	io0_(
		}t)l dicr	=	*st	V(=3(	,== olorDh
}	

a
	std::pair<int, int>lmax_colorD=S*std  max_element(m.begin(), m.end(),
 
	   i1(V>i>2IedVn<in ,pi<t>c;p n co;so:pa
	[- [](co-st pa-r) I	d nn &&  -,i<inn  ; i<
}
in;
rn nn &cil in / ; 	8SV</  =rScV(v ; /  ; n7& n; -  n n 17i> in
iS=     r
 rrS
}
m.sxto : d.ftpo.s_o < nds <
});
hsvr u = 3max_ oor.first
}
