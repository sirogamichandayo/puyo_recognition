...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*irage, llllllresize_it go, sssssscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
...
imv::v, >m:(dit:: .o (, al;
i#
}m::doxRe du eP uyoColor(oCnst HSVourn:cp2)({3u5(<c hsv->h && (hsonst HSV urn:cp2)({3u5 <= hsv->h && (hs
					
v    ->h && (hsv->h <= 135 &  120 urn: olhr::RE D;RED	rrci  5(<=->h ->h &&return hol= 70 &&or:: < hsv->v)BLUE;


v->hr&& eturn color2:5PURP15Eurn:
o:{
		 s35(v-->s  40))
return	o lo370::Por::U
...
((16PURPIE
			...
			u				 					rr<urn hR60 < S V *cop2t8{
			rn
					: olir::RED hsv)
											 RED;
				{
			iv) (160 <= hsGREENf&&  ((hsv->s <2450))
1			i5	t35(u)-rn olor:BLUWITE
 (2(0(		2lo070::P hsvU

	f ((16->h &&hsv->h <= 370)) || hsv-> > 60h&& hsv->h <= 30)) && hsv->v > 60)
			return o34orYELL:WUPD
reur85 <= n colh && (hov->hr:=:13OTER;
			
i((3n Stte::toR&P UL(PGe;
iSv)(160 <= hsGREENa&& 4te::to5ead2l5ueRed(const HSV *const hsv)
15{
						if) ((hsv-> BLU<40))
	reu2r0n color::WHITDnt Stat=e:60) || (t <= hsv->hoRe hsv->h <=a3r))e&&nYellow > 60const HSV *const hsv)
							{
f (550<h34-THhPURP  (	return color::YELLOW;
			if (tte::toR&P UL(PGe;
in(Sa490 < h5eadBluesvd(const HSV *const hsv)
{
									if ((hsv->s < 40))
			r>turn color::WHITGe&nY llowhsv->h < 120) && hsv->v > 100)
...
(		e5tun coo Y THLLO:;
	...
	if (79)0 <h>vs)&&;
 6i((3n Stte::toReGeD
			if (=0<6() || ( 90= hsv->h<   hsv->hh<=s3))>&&re & nY  l 60lowhsv->h < 120) && hsv->v > 100)
			retu50 < rn c THhR) &;

									if ((90 < hsv->h) && &(hlv->hor : 7GR && hsv->v > 200EEN;

																				 return < hsv->Gre & nY llowhsv->hco 120) &&
													lor
											: Y > v E 100)
										retu50 < rn coLL && (hlv->hor : 7GRO
																				 : hsv->v > 200EEN;

																				 return color
																				 : YELLO
																				 : (hsv->h < 120) && hsv->v > 100) return color::GREENR;
			}
							}
					R
