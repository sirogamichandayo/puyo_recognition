...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::Mat resize_image;
	cv::resize(*image, llllllllllllresize_image, sssssssssssscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
	elize(*im::e,cmlsi
	elize(*im::e,cmlsi
...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0);
...
= m n n 3tm hSS == n 3tm rr r r Sr r r S
											 S = n =
			= 3tm r r r S
}
in;
...
v)rrr
		vt
				m fm r=rtfm m  rnf r3tmmt1r
vsi)Stm(t1)ir> mfftm trfS
mm
v)rrr fm r=rtfm m  rnf r3tmmt1r
mm
v)rrr fm r=rtfm m  rnf r3tmmt1r
vs)eSftm(t1))rm ftm ttrmftmrt1rfrrt Sftttt1rsieSmtm(t1))rmr ttr  fSttm t1rm t rmftv t1rrt1r rrt Sftstt1rsieSftm(t1))rmrftm t rSftm t1rm t rSftm t1r)eSftm(t1))rm ftm ttrmftmrt1rfrrt Sftttt1rsieSmtm(t1))rmr ttr  fSttm t1rm t rmftm t1rrt1r rrt Sftmtt1rsieSftm(t1))rmrftm t rSftm t1rm t rSftm t1r
}t
mt
