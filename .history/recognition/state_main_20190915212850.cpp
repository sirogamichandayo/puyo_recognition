...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0);
	...
	{
		cv::Vec3b *p = &finish.at<cv::Vec3b> 2y, 0or(in = x *=)(*x < cols; ++x)
																								 m
																										 HtSx V 0;
		s hav.= = ooedPixe lCord & hsvx(&hsv);
			hsv.h(in(int)	*p.[0] * 2v =  = ()(*(*p)[2];

			color_pixel_dict[t> ax_co or =
