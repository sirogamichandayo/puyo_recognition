...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::Mat resize_image;
	cv::resize(*image, llresize_image, sscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
	int rewize(*im::e,rmwsi
	int cols = resize.cols;
#pragma omp parallel forish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0);
	for (int y = 0; y < rows; ++y)
	{
		...
		Pixe lCord &hsvx(&hsv);
		{
			...
			hsv.v . vi
	i *c)[2	*	)[2	V(=3(	,== olorDh
s	td::pair<int, int>lmax_colorD=S*std  max_element(m.begin(), m.end(),
 
a   i1(V>i>2IedVn<in ,pi<t>c;p n co;so:pa
	[- [](co-st pa-r) Ild nn &&  D,S<inn  ; i<
		}
		in;
...
n;
rn n 2cil in / ; 	8SV</  =rScV(vi;o/ r;rn7&Sn;
}
-
		hns = d .7i > in
											iS = r
