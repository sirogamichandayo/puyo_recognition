...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	std::map<int, int>
			color_pixel_dict = {
					...
					{color::PURPLE, 0}};
	cv::Mat resize_image;
	cv::resize(*image, lllllresize_image, ssssscv::Size(), 0.1, 0.1);

	int rows = resize_image.rows;
	int cols = resize_image.cols;
#pragma omp parallel forish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0);
	...
	return p1.secondeeeee <p2.second; });
