...
#pragma omp parallel for < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0);
	for (int y = 0; y < rows; ++y)
	{
		...
		Pixe lCord &hsvx(&hsv);
		{
			HSV hv(	((v.h* 2tvt*1svt 0*1svtV)tV);
					v.s =
(intsv*p [1(io
		}0]0pst
(iinun1i0sst
v.];inun i*s)[2v.
l=(&hs<)];77 && 150 < *) nst hsv) sv = neweHSVt);

sm	s   max	(:nor	=	*st	V(=3(	,== olorDh
					v =  &501= 77 && 250m , *D7i
 S5		vp=i	15
	
	stdo=:77n&&dr50 <t*l1( soi2, DeS< i< &{S 7>&7p2r5it		p  /nt81mVo= d):
t
		  p2  /   	8SV(nt,=rnt>& p;, c nst7pair intn int>&>p2)
			{
				< I(dV = r & retur cp1v < / c; n < n2 - srco 1;);
				< r1tuS = 3max_ o or.first
			}
cov); });
i	< Stat5::5oReadPixelP yoColo&(<oh t HSVi*rinst hs HV();
{
			rreturn color::NONE;
			//oHiV *	sv;10new50 < 1	0 = 0(v->N<DIST(h= atei	x hsv);sv->7Di (35 <1 v3407DPU15PL(s	i		r3)trn color::PURPLE;
			> hY(LHSV(hv)
							 ire);
			rn color::NONE;

			return color::PURP
								 in Stat = e1 : 5oRead2ue 
