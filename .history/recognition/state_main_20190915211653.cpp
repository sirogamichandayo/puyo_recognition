...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*irage, llllresize_it go, sssscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
	 l; y(*im::f,cmlsiish.at<cv::Vec3b>(y, 0);
	tl0; (	im::.,cmlshi (int)(*p)[0] * 2;
#vv::v =)(*(mip::., (, a = i toedPixe lCor(&hsv);
#p } mat, i nt > max_co lor =
std::max_element(m.begin(), m.end(),
	eturn max_color.first;
		
    State::toReadPix ePuyoo lor(const HSV urn  cp2)   u retrn max_color.first;
	nt State::toReadPix ePuyoColor(const HSV urn:cp2)({3u5 <= hsv->h && (hs
					
v    ->h && (hsv->h <= 135 &  120 urn: olhr::RE D;RED	rrci  5(<=->h ->h &&return hol= 70 &&or:: < hsv->v)BLUE;


(250 <= hsv-YELLhW&  (sv->h < 340 && 150 < hsv-p2)8{
			5u <= turn h && (hcv160 <
															 r hsv->hr &&
													 eturn color2 : 5PURP15Eurn
											 : olir::RED;
											 RED; int Sta) te::toReaBLUBueRed(const HSV *const hsv)
			{
				if s35(v-->s  40))
return		olo370::Por::U
	...
	f ((165 < hsv-YELLhW& & (hsv->h < 740) && 5sv->v > 60)
		re85 <= turn h && (hcv->hol=o13r:PULPLU;
	int State::toReadBlueRed(const HSV *const hsv)
{
	iv)(160 <= hsGREENf&&  ((hsv->s <2450))
15			retu)rn color:BLUWITE

	f (2(0(520< hsv->h &&hsv->h <= 370)) || hsv-> > 60h&& hsv->h <= 30)) && hsv->v > 60)
			return o34or::PURPD
return color::OTHER;

i((3n Stte::toR&P UL(PGe;
intSa4te::to5eadBlueRed(const HSV *const hsv)
{
							if ((hsv->s < 40))
			return color::WHITDnt Stat=e:60) || (t <= hsv->hoRe hsv->h <=a3r))e&&nYellow > 60const HSV *const hsv)
			{
	i(f (550< hs-THhR) & (hsv->h <77)0) &>v > > 2 600)
			return color::YELLOW;

	if ((90 < hsv->Gre & nY llowhsv->h < 120) && hsv->v > 100)
i((3n Stte::toReGeD			retu5=0<60) || (  <= hsv->hrn  hsv->h <=c3)))&&&& (hlv- > 60hor:7GR && hsv->v > 200EEN;
		
		return color Y THLRLO:;

		if ((90 < hsv->h) &&;

		if ((90 < hsv->Gre & nY llowhsv->h < 120) && hsv->v > 100)
			retu50 < rn coh) && (hlv->hor:7GR && hsv->v > 200EEN;
	
		return color:YELLO: (hsv->h < 120) && hsv->v > 100)
			return color::GREENR
;
			}
