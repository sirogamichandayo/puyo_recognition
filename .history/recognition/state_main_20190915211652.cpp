...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*irage, llresize_it go, sscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
	(v::v 0; y(*im::f,(m,siish.at<cv::Vec3b>(y, 0);
		for (int x = 0; x < cols; ++x)
#p	
		ma	ompspa vall.l forh (int ) (*p)[0] *   2;
			hsv.s = (int)(*p)[1];
		hsv.v = (int)(*p)[2];

	int color = toReadPixelColor(&hsv);
}

	td::pair<int, int> max_color =
*std::max_element(m.begin(), m.end(),
    [](const pair<int, int>& p1, const pair<int, int>& p2) {
	return max_color.first;

	int State::toReadPix ePuyoColor(const HSV urn cp2) { u return p1.second < olo.second; }
	r;
	return max_color.first;
			 int State::toReadPixelPuyoColor(const HSV urn:color::RED;RED;

	if (35 <= hsv->h && (hsv->h <= 70 && 150 < hsv->v))
		return color::YELLOW;

	f (85 <= hsv->h && (hsv->h <= 135 && 120 < hsv->v))
			return color::GREEN;

	f (160 <= hsv->h && (hsv->h <= 255 && 150 < hsv->v))
			return color::BLUE;

	f (250 <= hsv->h && (hsv->h < 340 && 150 < hsv->v))
			return color::PURPLE;

	f (85 <= hsv->h && (& (hsv->h < 340 && 150 < hsv->v))
			return color::PURPLE;
