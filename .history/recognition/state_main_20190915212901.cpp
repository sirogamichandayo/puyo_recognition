...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0);
	for (int y = 00 y < rows; ++y)
	{
		cv::Vec3b *p = &finish.at<cv::Vec3b>(y, 0);
		fm
		{
			tHx tS0;
			hav.= = ooedPixe lCord & hsvx(&hsv);
			hsv.hsvn(int) * p.[0] * 2v = = ()(*(*p)[2];

																				hcolh = ((nt)(*p)[0]) * 2ntsv)

					= So
		}(*}; uh> ax_=
 ;toRtuv noClE	if	(:nlDeirtrc0olor:2DIST			REh
}

std::pair<int, int> max_color = *std::max_element(m.begin(), .end(i, = olor(hsv = -15 <= 77 && 150 < *) nst hsv) sv = neweHSVt);
[](const pair<int, int> &p, censt; pai<int, int> & p2 / { t8SV(= 3Dl return &&sec n2<p2 - s> cov); });
