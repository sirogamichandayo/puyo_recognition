...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::Mat resize_image;
	cv::resize(*image, lllresize_image, ssscv::Size(), 0.1, 0.1);
	ols = resize.cols;
	elize(*im::e,cmlsi
	elize(*im::e,cmlsi
	int rewize(*im::e,rmwsi
	int cols = resize.cols;
#pragma omp parallel forish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0); ish.at < cv::Vec3b > (y, 0);
	for (int y = 0; y < rows; ++y)
	{
		...
		Pixe lCord &hsvx(&hsv);
		{
			...
			hsv.s = (int)(*p)[1];
			hsv.v . vi
	i *c)[2	*	)[2nt>lmax_colorD=S*[- [](co-st pa-r) Ild nn &&  D,S<inn  ; i<
		}
		in;
		rn nn &cil in / ; 	8SV</  =rScV(v ; /  ; n7& n; -  n n 17i> in
si	S= i1 V>i>2IedVn<i
a,n&n >c;p dico;so:>a
	[-n[](
		i-=    -r)lI	d nn &&D S, 
		 nr  ;
