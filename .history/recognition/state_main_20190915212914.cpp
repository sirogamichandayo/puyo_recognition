...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0);
  ...
  =sv.v		}(i= Su
}0]p];dict[tuv adPlxelC	(:n(&hsv)];
}
}
s
		td::pair<int, int>
				max_color = *std::max_element(m.begin(), .end(i, = olor(hsv = -15 <= 77 && 150 < *) nst hsv) sv = neweHSVt);
		 [](const pair<int, int>& p	, censt;pai
	<hsv)eweHSVt);
		 {t8V(=3(i, = olorDhsv =  15 o= 77 && r50 < *lm nst .,
Dsh< {
		t8V(= 3(i, = olorDhsv - 115 o = 7 && r150 < *l nt < hs) is
						vet);
		, inp1.&po / d {t8.= el
												nd teSV(; = rn && sec n2 < p2 / t8SV(= 3Dl return &&sec n2<p2 - s> cov); });
iri
urn.max_col(r.first;10 =S
	
0 = 0(v->N<DIST(h=v - h 1=3255&& 125 < hsv->vv))
int StateiitooeadP	xelP50 < 1or(const HSV *const hsv)
{
			// HSV *hsv = new HSV();
