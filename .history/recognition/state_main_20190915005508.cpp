...
void State::getState(const int &mode, std::vector<int> *const field)
{
	...
	if (mode == get_mode::allPuyo_1p)
	{
		...
		col_num = 1;
		row_num = 2;
		cv::Mat img_next_1p(img, pic::next_1p);
		std::vector<cv::Mat> img_next_1p_vec;
		std::vector<int> next_1p_vec;
		...
		col_num = 1;
		row_num = 2;
		cv::Mat img_next2_1p(img, pic::next2_1p);
		std::vector<cv::Mat> img_next2_1p_vec;
		std::vector<int> next2_1p_vec;
		splitImage(&img_next1_1p, col_num, row_num, &img_next2_1p_vec);
		// process
	}
...
void State::getNext_1p(std::vector<int> *field)
{
	...
	cv::Mat img_(img, pic::next2_2p);
	std::vector<cv::Mat> img__vec;
	splitImage(&img_, &col_num, &row_num, &img__vec);
}
