...
#pragma omp parallel forish.at < cv::Vec3b > (y, 0);
	for (int y = 0; y < rows; ++y)
	{
			...
			Pixe lCord &hsvx(&hsv);
		{
			HSV hsv;
			hsvsh = (int)(*p)[0] * 2;
			h]v.  = sint)(*p)[1];=n(unt)(npi*s];
2v.2v.
col ripixe*_)[2t[toReadPixelColor(&hsv)];
		}10
t     (:n;
	(}t)(:nor	=	*st	V(=3(	,== olorDh
}

	std::pair<int, int>lmax_colorD=S*std :max_element(m.begin(), m.end(),
    n1(i>i>2IedVn<int,pi<t>c;p1 nco;stopa
r<- t, i t>i p2) {
	[] - / netu < n p1Iedcnn & < 2iceco; 
	}en;rn  n 1cil in 	/  ; 	8SV</  =rScV(v ; /  ; n7& n; -  n n 17i> in
<
  1i>S =
int State::toReadPixelPuyoColor(const HSV *3o<s< hsvr u =3max_ oor.first 
{
	V;
	(7v3v)1 407 PS iPh(s = ir3
										 : H Vf);
	//unoHSV(rgb,< 50 &
	if (h & 12v->
			: P 0)
