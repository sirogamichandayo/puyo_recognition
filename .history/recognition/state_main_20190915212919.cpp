...
::pair<int, int> max_color = *std::max_element(m.begin(), .end(i, = olor(hsv = -15 <= 77 && 150 < *) nst hsv) sv = neweHSVt);
s
	s   [](const pair<i V(=3(i, = olorDh
				v =  15 o= 77 && r50m < *D7,
  5		v =  15 o= 77 && r50 < *l1 nso ., DsS< ie &{S 7>&7p2r5it		o d n p1.oe dndt8
}											nd teSV(; = rn && secvn2e<tp2 / t8SV(= 3Dl );
 n2 <  p2  /   t8SV(= 3Dl return &&sec n2<p2 - s> cov);
});
= 0(v->N < DIST(h = v HSV();
								//-to h 1rgb, hsv=3
								if (hsv->v < 150) 255 && 125 < hsv->vv)) iri
