...
int toGetPuyoColorPerPiece(const cv::Mat *const image)
{
	...
	cv::resize(*irage, lresize_it go, scv::Size(), 0.1, 0.1);
	ols = resize.cols;
	cv::vesize(*im::e,(m,si
	for (int y = 0; y < rows; ++y)
#pc *pma omp pa &allfl forish.at < cv::Vec3b > (y, 0);
...
std::pair<int, int> max_color =
		*std::max_element(m.begin(), m.end(),
											[](const pair<int, int> &p1, const pair<int, int> &p2) { u return p1.second < p2.second; });
return max_color.first;
			 int State::toReadPixelPuyoColor(const HSV urn color::RED;
